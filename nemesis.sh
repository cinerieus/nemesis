...
if echo "$server" | grep -iqFv y; then
	pacman --noconfirm -S xorg-server plasma-meta kwalletmanager kvantum dolphin kwrite gwenview konsole spectacle chromium firefox-developer-edition libreoffice remmina
...
if echo "$extra" | grep -iqF y; then
	## tools ##
	pacman --noconfirm -S nmap impacket metasploit sqlmap john medusa ffuf enum4linux-ng linux-smart-enumeration seclists bloodhound-python ldapdomaindump ntdsxtract binwalk evil-winrm responder freerdp gowitness miniserve cewl strace pspy gittools scoutsuite pacu subfinder httpx dnsx gau nuclei interactsh-client asnmap
	#pip install coercer
	## extra ##
	umask 002
